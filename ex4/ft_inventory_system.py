    ...
    for arg in sys.argv[1:]:
        ...
        for i in range(len(arg)):
            if arg[i] == ':':
                ...
                break
        if colon_index != -1:
            item_name = arg[:colon_index]
            item_quantity_str = arg[colon_index + 1:]
            inventory.update({item_name: int(item_quantity_str)})
...
for key, value in inventory.items():
    if checker < len(inventory) - 1:
        ...
        print(value)
    checker+=1
# you can check if a key exists in a dictionary using:
key = "sword"
if key in inventory:
