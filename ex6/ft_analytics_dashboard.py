...
if __name__ == "__main__":
    ...
    hi_scores = [
        p for p in a["players"]
        if a["players"][p]["total_score"] > 2000
    ]
    print("High scorers (>2000):", hi_scores)

    double_scores = [
        a["players"][p]["total_score"] * 2 for p in a["players"]
    ]
    print("Scores doubled:", double_scores)

    active_players = [
        p for p in a["players"]
        if a["players"][p]["sessions_played"] > 15
    ]
    ...
    player_scores = {
        p: a["players"][p]["total_score"] for p in a["players"]
    }
    print("Player scores:", player_scores)

    score_list = [
        "high" if s["score"] >= 2500 else
        "medium" if s["score"] >= 1500 else "low"
        for s in a["sessions"]
    ]
    ...
    achievements = {
        p: a["players"][p]["achievements_count"] for p in a["players"]
    }
    ...
    unique_ach = {ach for ach in a["achievements"]}
    ...
    top_player = [
        p for p in a["players"]
        if a["players"][p]["total_score"] == biggest_score
    ]
    top_achievement_count = max([
        a["players"][p]["achievements_count"] for p in a["players"]
    ])
    print(
        f"Top performer: {top_player} ({biggest_score} points, "
